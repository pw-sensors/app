...
pw::Status test::sensor::Run(pw::sensor::Sensor &sensor,
  ...
  struct future_context {
    pw::sensor::SensorFuture future;
    pw::sensor::Sensor *sensor;
  } future = {
      .future = sensor.Read(ctx, {read_list}),
      .sensor = &sensor,
  };
  pw::async::Task task([&future](pw::async::Context &task_context,
    ...
    auto result = future.future.Poll(waker);
    if (result.IsReady()) {
      ...
      PW_LOG_INFO("Resetting decoder: %s",
                  decoder.Reset(result.value().value().as_byte_span()).str());

      auto size_info = decoder.GetSizeInfo(pw::sensor::ACCELEROMETER);
      ...
      auto frame_count = decoder.GetFrameCount(pw::sensor::ACCELEROMETER, 0);
      PW_ASSERT_OK(frame_count.status());
      PW_LOG_INFO("Found %zu ACCELEROMETER frames", frame_count.value());

      std::byte out[64];
      auto decode_result =
          decoder.Decode(pw::sensor::ACCELEROMETER, 0, 1, pw::ByteSpan(out));
